package com.example.SpringBatchTutorial.job.customStep.conditionalStep;
 ...
 * run env: --spring.batch.job.namesconditionalStepJob
 */
@Slf4j
@Configuration
...
//                        log.info("conditional Start Step");
    ...
    public Step conditionalFailStep() {
        return stepBuilderFactory.get("conditionalFailStep")
                .tasklet(new Tasklet() {
                    @Override
                    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
                        log.info("conditional Fail Step");
                        return RepeatStatus.FINISHED;
                    }
                })
                .build();
